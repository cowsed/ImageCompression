...
@app.cell
def _(deoctree, octree, remake_image, sub_part, subdiv_img):
    def octree_generic(level, img, f_split, f_last):
        if img.shape[0] == 0 or img.shape[0] == 0:
            raise ValueError("went too far, out of pixels")
        if level == 0:
            return f_last(img)
        ...
        return me, subos
    def deoctree_generic(level_part, sub_parts, full_shape, ttl, f_join, f_last):
        half_shape = (full_shape[0]//2, full_shape[1]//2)
        if not isinstance(sub_parts[0], tuple):
            return f_last(level_part, sub_part) 
            # m+remake_image(*list(map(lambda me : np.full(half_shape, me), subs)))

        # if ttl < 1:
        #     tl, tr, bl, br = tuple(map(lambda a : np.full(half_shape, a[0]), subs))
        #     return remake_image(tl, tr, bl, br)        
        parts = tuple(map(lambda a : deoctree(a[0], a[1], half_shape, ttl - 1), sub_parts))
        return f_join(level_part, remake_image(*parts))
    
    
    return deoctree_generic, octree_generic


@app.cell
def _(full_shape, np, remake_image):
    def time_domain_f_split(img):
        return None
    def time_domain_f_last(shape, m, subs):
        half_shape = (full_shape[0]//2, full_shape[1]//2)
        return m+remake_image(*list(map(lambda me : np.full(half_shape, me), subs)))
    # def time_domain_f_join(me, parts):
    return (time_domain_f_last,)
...
def _(
    ...
    m, subs = octree(8, Y)
    m1, subs1 = octree_generic(8, Y, remove_mean, lambda i : i.mean())
    rimg = deoctree(m, subs, Y.shape, 10)
    rimg1 = deoctree_generic(m, subs, Y.shape, 10, lambda x,y : remake_image(*y)+x, time_domain_f_last)
